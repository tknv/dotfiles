...
http_prgs="/usr/local/bin/surf:PW /usr/bin/uzbl-browser:PW /usr/bin/sensible-browser:PW /usr/bin/sensible-browser:XT /usr/bin/galeon:PW /usr/bin/konqueror:PW /usr/bin/mozilla:PW /usr/bin/lynx:XT /usr/bin/links:XT"
https_prgs=$http_prgs
mailto_prgs="/usr/bin/mutt:VT /usr/bin/elm:VT /usr/bin/alpine:VT /usr/bin/pine:VT /usr/bin/mail:VT"
gopher_prgs="/usr/bin/forg:XT /usr/bin/gopher:XT /usr/bin/lynx:XT"
ftp_prgs="/usr/bin/curl:XT /usr/bin/ncftp:XT /usr/bin/lftp:XT $http_prgs"
...
#    prg=`getprg $https_prgs`
     prg=`~/Scripts/url-surf-tabbed.sh $url > /dev/null 2>&1 &`
     exit
    ;;
http)
